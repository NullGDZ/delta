...
class GUI {
	...
	updateRows() {
		if (this.rowsinit) {
			...
			// Change "Mass" row to "Status" row
			this.divs.rows.massbots.innerHTML = `<span>Status: <span id="bot_status" style="color: ${window.startbot ? '#27ae60' : '#e74c3c'}">${window.startbot ? "Started" : "Stopped"}</span></span>`;
			...
			for (const t of Object.keys(this.inputs)) {
				if (t !== "massbots") { // Skip massbots since we've repurposed its row
	...
	// Add a new method to update bot status display
updateBotStatus() {
		...
		document.body.addEventListener("keydown", t => {
			...
			if (document.getElementById('message-box').style.display == 'none'){
				if (!(8 === e || t.ctrlKey || t.shiftKey || t.altKey)) {
					...
					// Repurpose massbots hotkey to toggle bot status
					if (e === this.getKey(this.keys.massbots)) {
						if (this.isActive(this.keys.massbots)) return;
						this.active.add(this.keys.massbots);
						// Toggle bots on/off
