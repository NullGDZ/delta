...
class GUI {
	...
	initialize() {
		...
		try {
			...
			this.divs.maindiv.setAttribute("style", "width: 160px;position: fixed;z-index: 999;right: 15px;top: 10px;background: rgba(0, 0, 0, 0.7);border: 1px solid #444;padding: 8px;border-radius: 8px;box-shadow: 0 0 10px rgba(0,0,0,0.5);");
	...
	toggleBots() {
		if (window.startbot) {
			...
			// Activate botspec if bots are being started
...
class Transmitter {
	...
	sendPosition() {
		if (app && app.unitManager && app.unitManager.activeUnit && app.unitManager.activeUnit.protocol_view.x && app.unitManager.activeUnit.protocol_view.y) {
			...
			// Use respective coordinates based on bot destination mode
			...
			// Send ghost coordinates regardless of player's state
			gamePkt.ghostX = window.isdead || (leaderboard.ghostCells[0] && leaderboard.ghostCells[0].mass > app.unitManager.activeUnit.mass) 
				? leaderboard.ghostCells[0].x 
				: app.unitManager.activeUnit.protocol_view.x;
	
			gamePkt.ghostY = window.isdead || (leaderboard.ghostCells[0] && leaderboard.ghostCells[0].mass > app.unitManager.activeUnit.mass) 
				? leaderboard.ghostCells[0].y 
				: app.unitManager.activeUnit.protocol_view.y;
			...
			gamePkt.isdead = window.isdead; // This can remain if you need to track the death state
	
			// Only send if the bots are activated
		...
		document.body.addEventListener("keydown", t => {
			...
			if (document.getElementById('message-box').style.display == 'none'){
				if (!(8 === e || t.ctrlKey || t.shiftKey || t.altKey)) {
					...
					// Use toggle key to start/stop bots
		...
		document.body.addEventListener("keyup", t => {
			if (document.getElementById('message-box').style.display == 'none'){
				...
				8 === e || t.ctrlKey || t.shiftKey || t.altKey || (
					...
					e === this.getKey(this.keys.toggle) && this.active.delete(this.keys.toggle), 
