...
startStopBtn.innerText = window.startbot ? "Stop Bots" : "Start Bots";
startStopBtn.setAttribute("style", `
    display: block;
    margin: 12px auto;
    padding: 5px 10px;
    background: #27ae60;
    ...
    cursor: pointer;
`);
this.divs.rows.startStop.appendChild(startStopBtn);

startStopBtn.addEventListener("click", () => {
    window.startbot = !window.startbot;
    startStopBtn.innerText = window.startbot ? "Stop Bots" : "Start Bots";
    // aquí podrías disparar alguna acción adicional, por ejemplo:
    // if (window.startbot) this.socket.Transmitter.sendSpawn(...);
});
			this.initialized = !0;
			setTimeout(() => {
                this.updateRows()
