...
// Create instances for two accounts
const accountKeys = [
    '484522_32287c3a692abb5bfd2aa7589b18c26131ee4e5d', // Account 1 key
    '484522_d2bd1d1d0eb1273e70af7ad9c94c4174a7c78ea5' // Account 2 key
];
window.botr = false;
class GUI {
    constructor(t, instanceNumber) {
          this.socket = t,
          this.instanceNumber = instanceNumber, // Instance-specific data
...
class AgarBot {
    constructor(accountKey, instanceNumber) {
        this.ip = "";
        this.ws = null;
        this.instanceNumber = instanceNumber; // Unique instance number
        this.player = {
            isPremium: false,
            PremiumType: 0,
            PureFeeder: false,
        ...
        this.GUI = new GUI(this, instanceNumber); // Pass instance number
        ...
        this.get = function (t, e) {
            ...
            s.open("GET", t), s.send(), s.onload = function () {
                200 != s.status ? alert("Response failed") : e(s.responseText);
            }, s.onerror = function () {
                alert("Request failed");
            }
        };
        this.connect(`wss://gamesrv.agarbot.ovh:8443/?key=${accountKey}`); // use accountKey
    }
    connect(t) {
    ...
    onopen() {
        ...
        console.log(`[AGARBOT ${this.instanceNumber}] Authenticating to the server!`);
        this.Transmitter.handshake("220720", "Delta");
    }
    send(t) {
...
let agarBotInstances = [];
accountKeys.forEach((key, index) => {
    agarBotInstances.push(new AgarBot(key, index + 1)); // Index used as instance number
});
