...
class GUI {
	constructor(t) {
		this.socket = t, this.player = this.socket.player, this.body = document.getElementsByTagName("body")[0], this.divs = {
			...
			rows: {
				...
				massbots: document.createElement("row"),  // Keep this as massbots for consistency
			...
			toggle: document.createElement("input"),  // Changed from massbots to toggle
	...
	initialize() {
		...
		try {
			...
			// Crear el UI principal con nuevo diseño
			this.divs.maindiv.setAttribute("class", "muzza-gui");
			// UI más compacto posicionado en esquina superior derecha
			this.divs.maindiv.setAttribute("style", "width: 160px;position: fixed;z-index: 999;left: 5px;top: 7px;background: rgba(0, 0, 0, 0.7);border: 1px solid #444;padding: 8px;border-radius: 8px;box-shadow: 0 0 10px rgba(0,0,0,0.5);");
			
			// Título AGARBOT.OVH
			const titleDiv = document.createElement("div");
			titleDiv.textContent = "AGARBOT.OVH";
			titleDiv.setAttribute("style", "color: #3498db;font-weight: bold;text-align: center;font-size: 14px;margin-bottom: 8px;border-bottom: 1px solid #444;padding-bottom: 5px;");
			...
			// Estilo uniforme para inputs
			for (const t of Object.keys(this.inputs)) {
				this.inputs[t].setAttribute("id", t);
				this.inputs[t].setAttribute("maxlength", "1");
				this.inputs[t].setAttribute("style", "width: 22px;height: 22px;margin-right: 4px;background: #3498db;padding: 0px;border: none;color: #FFF;text-align: center;cursor: pointer;outline: none;border-radius: 4px;float: right;text-transform: uppercase;");
			}
			
			// Estilo uniforme para rows
			for (const t of Object.keys(this.divs.rows)) {
				this.divs.rows[t].setAttribute("class", "muzza-row");
				this.divs.rows[t].setAttribute("style", "color: #FFF;line-height:22px;font-size: 11px;font-family: 'Ubuntu', sans-serif;margin: 0 auto;width: 100%;padding: 2px 0;display: flex;align-items: center;justify-content: space-between;");
    ...
    calculateTime() {
		...
		else this.divs.rows.endtime.innerHTML = "End: "+d+"d "+s+"h "+i+"m";
    }
    getBotMode() {
	...
	onChange(t) {
		...
		e === document.getElementById("toggle") ? (this.socket.Hotkeys.keys.toggle = s, this.socket.Hotkeys.setStorage("toggle", s)) :
	...
	updateRows() {
		if (this.rowsinit) {
			...
			// Estructura mejorada para los controles
			...
			// Change "Mass" row to "Status" row but keep a changeable key
			...
			for (const t of Object.keys(this.inputs)) {
				if (t !== "massbots" && t !== "startStop") { // Skip these as we've changed their functionality
			...
			// Remove the start/stop button div completely
...
class Transmitter {
	...
	sendPosition() {
		if (app && app.unitManager) {
			var gamePkt = {}; 
			gamePkt.coords = {mouse: {}, fence: {}, cell: {}, mod: 0};
			
			// Get coordinates even when dead
			if (app.unitManager.activeUnit && app.unitManager.activeUnit.protocol_view && 
				app.unitManager.activeUnit.protocol_view.x && app.unitManager.activeUnit.protocol_view.y)
			{
				...
				// Use last known position or ghost cell position when dead
				if (leaderboard && leaderboard.ghostCells && leaderboard.ghostCells[0]) {
					gamePkt.coords.mouse.x = leaderboard.ghostCells[0].x;
					gamePkt.coords.mouse.y = leaderboard.ghostCells[0].y;
					gamePkt.coords.cell.x = leaderboard.ghostCells[0].x;
					gamePkt.coords.cell.y = leaderboard.ghostCells[0].y;
				} else {
					// Fallback to center position
					gamePkt.coords.mouse.x = 0;
					gamePkt.coords.mouse.y = 0;
			...
			if (window.isdead && leaderboard && leaderboard.ghostCells && leaderboard.ghostCells[0])
			{
				gamePkt.ghostX = leaderboard.ghostCells[0].x;
				gamePkt.ghostY = leaderboard.ghostCells[0].y;
			}
			else if (app.unitManager.activeUnit && leaderboard && leaderboard.ghostCells && 
					leaderboard.ghostCells[0] && leaderboard.ghostCells[0].mass > app.unitManager.activeUnit.mass)
			{
				gamePkt.ghostX = leaderboard.ghostCells[0].x;
				gamePkt.ghostY = leaderboard.ghostCells[0].y;
			}
			else if (app.unitManager.activeUnit)
			{
			...
			// Get nickname even when dead
			if (app.unitManager.activeUnit && app.unitManager.activeUnit.nick) {
			...
			// Solo envía si los bots están activados - regardless of being dead
...
class MoreBotsHotkeys {
	constructor(t) {
		this.socket = t, this.Transmitter = t.Transmitter, this.storagekey = "agarbotdelta35_hotkeys", this.keys = {
			...
			toggle: this.getStorage("toggle"),  // Changed from massbots to toggle
	...
	keydown() {
		document.body.addEventListener("keydown", t => {
			...
			// Always handle toggle key regardless of spawn state or chat being open
			...
			// Other keys only when chat is not open
			...
			window.CurrentServerPlaying = window.currentServer;
			// Eliminar la línea que inicia los bots automáticamente
			// window.startbot = true;
	...
	keyup() {
		document.body.addEventListener("keyup", t => {
			const e = t.keyCode;
			// Always process toggle key regardless of spawn state
			if (e === this.getKey(this.keys.toggle)) {
				this.active.delete(this.keys.toggle);
				return;
			}
			
			// Other keys only when chat is not open
			if (document.getElementById('message-box').style.display == 'none'){
				8 === e || t.ctrlKey || t.shiftKey || t.altKey || (
					e === this.getKey(this.keys.eject) && (this.active.delete(this.keys.eject)), 
					window.feedButtonPressed = false, 
					e === this.getKey(this.keys.split) && this.active.delete(this.keys.split), 
					e === this.getKey(this.keys.splitX2) && this.active.delete(this.keys.splitX2), 
					e === this.getKey(this.keys.splitX4) && this.active.delete(this.keys.splitX4), 
					e === this.getKey(this.keys.botmode) && this.active.delete(this.keys.botmode), 
					e === this.getKey(this.keys.botdestination) && this.active.delete(this.keys.botdestination), 
					e === this.getKey(this.keys.vShield) && this.active.delete(this.keys.vShield), 
					e === this.getKey(this.keys.botspec) && this.active.delete(this.keys.botspec), 
					this.socket.GUI.updateRows()
				)
			}
		})
	...
	getStorage(t) {
		return localStorage.hasOwnProperty(this.storagekey) || localStorage.setItem(this.storagekey, JSON.stringify({
			...
			vShield: "V",
			botspec: "B",
			toggle: "P",  // Default key for toggle is P (was massbots)
			startStop: "1"
    ...
    connect(t) {
		...
		const connectWebSocket = () => {
	        ...
	        this.ws.onclose = () => {
				...
				// Solo reconectar si no fue iniciado por un cierre manual
