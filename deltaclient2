...
class GUI {
    ...
    toggleBots() {
        ...
        if (window.isBotRunning) {
            botControlBtn.textContent = "Stop Bots";
            botControlBtn.className = "control-btn red";
            this.socket.Transmitter.sendSpawn();
        } else {
            botControlBtn.textContent = "Start Bots";
            botControlBtn.className = "control-btn green";
        }
        
        console.log("Bots " + (window.isBotRunning ? "started" : "stopped"));
    }
    ...
    updateRows() {
        if (this.rowsinit) {
            ...
            if (this.player.decoded.botsMax === undefined) {
                this.divs.rows.bots.innerHTML = `<div>Authenticating...</div>`;
            } else if (this.player.decoded.botsMax == 0) {
                this.divs.rows.bots.innerHTML = `<div>Bots: 0/0</div>`;
            }
            
            // Botón de control de bots
