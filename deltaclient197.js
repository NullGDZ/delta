...
// AGARBOT UI - Modified by Null
initialize() {
    // Reset current server tracker
    ...
    try {
        // Event listeners for spawn and death
        ...
        styleTag.textContent = `
            ...
            .null-toggle input { display:none; }
            .null-slider {
                position: absolute;
                cursor: pointer;
                top: 0; left: 0;
            ...
            .null-toggle input:checked + .null-slider {
                background-color: #4cd964;
            }
            .null-toggle input:checked + .null-slider:before {
        ...
        this.divs.maindiv.className = 'null-gui';
        this.divs.maindiv.innerHTML = '';

        // Header
        const header = document.createElement('div');
        header.className = 'header';
        header.textContent = 'AGARBOT.OVH Modified By Null';
        this.divs.maindiv.appendChild(header);

        // Data container
        this.divs.data.className = 'data';
        this.divs.maindiv.appendChild(this.divs.data);

        // Generate rows for each input
        Object.keys(this.inputs).forEach(key => {
            const row = this.divs.rows[key];
            row.className = 'row';
            ...
            input.id = key;
            input.maxLength = 1;
            input.className = 'keybind';

            // Append elements
            row.appendChild(label);
            row.appendChild(input);
            this.divs.data.appendChild(row);
        });

        // Example toggle row (e.g., Start/Stop)
        const toggleRow = document.createElement('div');
        toggleRow.className = 'row';
        const toggleLabel = document.createElement('span');
        toggleLabel.textContent = 'BOT';
        const toggleWrapper = document.createElement('label');
        toggleWrapper.className = 'null-toggle';
        const toggleInput = document.createElement('input');
        toggleInput.type = 'checkbox';
        toggleInput.onchange = () => { window.startbot = toggleInput.checked; };
        const toggleSlider = document.createElement('span');
        toggleSlider.className = 'null-slider';
        toggleWrapper.appendChild(toggleInput);
        toggleWrapper.appendChild(toggleSlider);
        toggleRow.appendChild(toggleLabel);
        toggleRow.appendChild(toggleWrapper);
        this.divs.data.appendChild(toggleRow);

        // Append to document
        document.body.appendChild(this.divs.maindiv);
        this.initialized = true;

        // Update rows after load
        setTimeout(() => this.updateRows(), 500);
    } catch (err) {
