...
class GUI {
    ...
    initialize() {
        ...
        try {
			...
			window.app.on("spawn", (tab) => {   
				...
				// Se eliminó el auto-inicio de bots al aparecer
			});
			window.app.on("death", () => {
					window.isdead = true;
			});
            this.divs.maindiv.setAttribute("class", "muzza-gui"), 
            // Estilo modificado para UI minimalista en medio-izquierda
            this.divs.maindiv.setAttribute("style", "width: 180px;position: fixed;z-index: 999;left: 15px;top: 3%;transform: translateY(-50%);background: rgba(0, 0, 0, 0.6);border: 1px solid #333;padding: 8px;border-radius: 10px;box-shadow: 0 0 10px rgba(0,0,0,0.5);"), 
            this.divs.img.setAttribute("class", "img"), 
            this.divs.img.setAttribute("style", "background-image:;width: 100%;height: 30px;background-repeat: no-repeat;margin: 0 auto;"), 
            this.divs.maindiv.appendChild(this.divs.img), 
            this.divs.data.setAttribute("class", "data"), 
            this.divs.data.setAttribute("style", "margin-top: 8px;width: 100%;"), 
            this.divs.maindiv.appendChild(this.divs.data);

            for (const t of Object.keys(this.inputs)) this.inputs[t].setAttribute("id", t), this.inputs[t].setAttribute("maxlength", "1"), this.inputs[t].setAttribute("style", "width: 28px;height: 28px;margin-right: 6px;background: #3498db;padding: 0px;border: none;color: #FFF;text-align: center;cursor: pointer;outline: none;border-radius: 50%;float: right;text-transform: uppercase;");
            for (const t of Object.keys(this.divs.rows)) {
                this.divs.rows[t].setAttribute("class", "muzza-row"), this.divs.rows[t].setAttribute("style", "color: #FFF;line-height:18px;font-size: 11px;font-family: 'Ubuntu', sans-serif;margin: 0 auto;width: 100%;margin-left: 6px;"), this.divs.data.appendChild(this.divs.rows[t]);
    ...
    updateRows() {
        if (this.rowsinit)document.getElementById("eject").value = this.socket.Hotkeys.keys.eject, document.getElementById("split").value = this.socket.Hotkeys.keys.split, document.getElementById("splitX2").value = this.socket.Hotkeys.keys.splitX2, document.getElementById("splitX4").value = this.socket.Hotkeys.keys.splitX4, document.getElementById("botgamemode").value = this.socket.Hotkeys.keys.botmode, document.getElementById("botdestination").value = this.socket.Hotkeys.keys.botdestination, document.getElementById("massbots").value = this.socket.Hotkeys.keys.massbots, document.getElementById("botspec").value = this.socket.Hotkeys.keys.botspec, document.getElementById("vShield").value = this.socket.Hotkeys.keys.vShield, document.getElementById("bot_mode_key").innerText = this.getBotMode(), document.getElementById("vshield_mode_key").innerText = this.getvShieldMode(), document.getElementById("massbots_mode_key").innerText = this.getmassbots(), document.getElementById("botspec_mode_key").innerText = this.getbotspec(), document.getElementById("botdestination_mode_key").innerText = this.getbotdestinationMode();
            ...
            this.divs.rows.splitX2.innerHTML = "SplitX2: " + this.socket.GUI.inputs.splitX2.outerHTML, 
            this.divs.rows.splitX4.innerHTML = "SplitX4: " + this.socket.GUI.inputs.splitX4.outerHTML, 
            this.divs.rows.botgamemode.innerHTML = `Mode: <span id="bot_mode_key">${this.getBotMode()}</span> ${this.socket.GUI.inputs.botgamemode.outerHTML}`, 
            this.divs.rows.botdestination.innerHTML = `Dest: <span id="botdestination_mode_key">${this.getbotdestinationMode()}</span> ${this.socket.GUI.inputs.botdestination.outerHTML}`, 
            this.divs.rows.vshield.innerHTML = `Shield: <span id="vshield_mode_key">${this.getvShieldMode()}</span> ${this.socket.GUI.inputs.vShield.outerHTML}`, 
            this.divs.rows.botspec.innerHTML = `Spec: <span id="botspec_mode_key">${this.getbotspec()}</span> ${this.socket.GUI.inputs.botspec.outerHTML}`, 
            this.divs.rows.massbots.innerHTML = `Mass: <span id="massbots_mode_key">${this.getmassbots()}</span> ${this.socket.GUI.inputs.massbots.outerHTML}`,
            ...
            for (const t of Object.keys(this.inputs)) this.inputs[t].setAttribute("id", t), this.inputs[t].setAttribute("maxlength", "1"), this.inputs[t].setAttribute("style", "width: 28px;height: 28px;margin-right: 6px;background: #3498db;padding: 0px;border: none;color: #FFF;text-align: center;cursor: pointer;outline: none;border-radius: 50%;float: right;text-transform: uppercase;"), document.getElementById(this.inputs[t].id).addEventListener("input", t => this.onChange(t), !1), document.getElementById(this.inputs[t].id).onclick = t => {
                t.target.select()
            };
            
            // Botón de inicio/detención de bots añadido
            this.divs.rows.botnum.innerHTML = "<div style=\"margin-top:12px;\" id=\"botnum\"></div>";
            const startStopButton = document.createElement("button");
            startStopButton.id = "startStopBotsButton";
            startStopButton.innerText = "START BOTS";
            startStopButton.style.width = "100%";
            startStopButton.style.padding = "6px";
            startStopButton.style.marginTop = "10px";
            ...
            startStopButton.style.fontWeight = "bold";
            startStopButton.onclick = () => this.toggleBots();
            this.divs.rows.botnum.appendChild(startStopButton);
	...
	toggleBots() {
		const button = document.getElementById("startStopBotsButton");
		if(window.startbot) {
			window.startbot = false;
			window.botr = false;
			button.innerText = "START BOTS";
...
class Transmitter {
    ...
    sendPosition() {
						...
						// Solo envía si los bots están activados
